...
export const Route = createFileRoute('/embed/artist/$id' as any)({ component: EmbedArtistPage })

function EmbedArtistPage() {
  ...
  const pictureUrl = artist.picture ? losslessAPI.getArtistPictureUrl(artist.picture, '750') : null
  ...
  return (
    <main className="relative flex h-screen flex-col overflow-hidden bg-slate-950 text-white">
      <div
        ...
        style={{ backgroundImage: pictureUrl ? `url(${pictureUrl})` : undefined, backgroundColor: pictureUrl ? undefined : '#1e293b' }}
      />

      <header className="relative z-10 flex items-center gap-3 bg-black/25 p-4 backdrop-blur-md">
        <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full shadow-xl bg-slate-700">
          {pictureUrl ? (
            <img src={pictureUrl} alt={artist.name} className="h-full w-full object-cover" />
          ) : null}
          <button
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-sm font-semibold" title={artist.name}>{artist.name}</h1>
          <p className="truncate text-xs text-slate-200/85">Top tracks</p>
          <a href={`/artist/${artist.id}`} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs text-blue-300 hover:text-blue-200">
