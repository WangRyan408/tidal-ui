...
function ensureVaryIncludesOrigin(value: string | null): string {
  const entries = value
    ? value
        .split(',')
        .map((v) => v.trim())
...
async function shouldRetryInvalidToken(response: Response, targetUrl: URL): Promise<boolean> {
  ...
  if (!isRetryEligibleTarget(targetUrl)) return false;
  ...
  try {
    ...
    if (subStatus === 11002) return true;
    if (combinedMessage.includes(TOKEN_INVALID_MESSAGE)) return true;
  } catch (error) {
...
export const Route = createFileRoute('/api/proxy' as any)({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const upstream = await fetchWithRetry(parsedTarget, {
